...
const STATUSES = ['New','In Progress','Archived'];
...
function nextFileSystemName(ext='file'){let count=0; state.tasks.forEach(t=>count+=(t.files||[]).length); return `${count+1}-v1-WW.${ext}`;}
...
function searchResults(){const q=state.query.trim().toLowerCase(); if(!q) return []; return state.tasks.filter(t=>[t.title,t.message,t.category,t.status,...(t.comments||[]).map(c=>c.text),...(t.files||[]).map(f=>`${f.filename} ${f.display_name}`)].join(' ').toLowerCase().includes(q));}
...
function downloadText(name, text){const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([text],{type:'application/json'})); a.download=name; document.body.appendChild(a); a.click(); a.remove(); setTimeout(()=>URL.revokeObjectURL(a.href),1000);}
...
document.querySelectorAll('.tiny-action').forEach(b=>b.onclick=()=>{state.view='all'; state.selectedId=null; render();});
